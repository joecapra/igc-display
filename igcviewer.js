(function ($) {
  ...
  function plotBarogram() {
    ...
    for (j = 0; j < nPoints; j++) {
      ...
      pressureBarogramData.push([
        timestamp,
        igcFile.pressureAltitude[j] * altitudeConversionFactor,
      ]);
      gpsBarogramData.push([
        timestamp,
        igcFile.gpsAltitude[j] * altitudeConversionFactor,
      ]);
    }
  ...
  function updateTimeline(timeIndex, mapControl) {
    ...
    $("#timePositionDisplay").text(
      moment(igcFile.recordTime[timeIndex]).format("HH:mm:ss") +
        ": " +
        (
          igcFile.pressureAltitude[timeIndex] * altitudeConversionFactor
        ).toFixed(0) +
        " " +
        unitName +
        " (barometric) / " +
  ...
  function displayIgc(mapControl) {
    ...
    for (headerIndex = 0; headerIndex < igcFile.headers.length; headerIndex++) {
      headerTable.append(
        $("<tr></tr>")
          .append($("<th></th>").text(igcFile.headers[headerIndex].name))
    ...
    if (igcFile.task.coordinates.length > 0) {
      ...
      if (igcFile.task.coordinates[0][0] !== 0) {
        ...
        if (igcFile.task.takeoff.length > 0) {
          taskList.append(
            $("<li> </li>").text("Takeoff: " + igcFile.task.takeoff)
          );
        }
        for (j = 0; j < igcFile.task.names.length; j++) {
          switch (j) {
            case 0:
              taskList.append(
                $("<li> </li>").text("Start: " + igcFile.task.names[j])
              );
              break;
            case igcFile.task.names.length - 1:
              taskList.append(
                $("<li> </li>").text("Finish: " + igcFile.task.names[j])
              );
              break;
            default:
              taskList.append(
                $("<li></li>").text(
                  "TP" + j.toString() + ": " + igcFile.task.names[j]
                )
              );
          }
        }
        if (igcFile.task.landing.length > 0) {
          taskList.append(
            $("<li> </li>").text("Landing: : " + igcFile.task.landing)
          );
        }
  ...
  $(document).ready(function () {
    ...
    $.each(moment.tz.names(), function (index, name) {
      timeZoneSelect.append($("<option></option>", { value: name }).text(name));
    });
    ...
    timeZoneSelect.change(function () {
      ...
      if (igcFile !== null) {
        ...
        $("#headerInfo td")
          .first()
          .text(moment(igcFile.recordTime[0]).format("LL"));
      }
    ...
    $("#fileControl").click(function () {
      ...
      xhttp.onload = function () {
        ...
        igcFile = parseIGC(this.responseText);
        displayIgc(mapControl);
      };
      xhttp.open(
        "GET",
        "https://firebasestorage.googleapis.com/v0/b/logbook-7cff5.appspot.com/o/27AVQ0J1.igc?alt=media&token=3597a3ad-7ba4-473d-bd9f-001f9aef9e3f"
      );
      xhttp.send();
